...
import { computeLoad, memberArc } from '../utils/load';
...
function BalanceDial({ persona, size = 180 }: { persona: Persona; size?: number }) {
    ...
    <View style={{ width: size, height: size }}>
      <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ position: 'absolute' }}>
        ...
        {persona.members.map((_, i) => {
          ...
          return (
            <Line key={i}
              x1={cx + Math.cos(ang) * rOuter}  y1={cy + Math.sin(ang) * rOuter}
      ...
      <AnimatedCard index={2} style={{ borderRadius: 18, borderWidth: 0.5, padding: 16, backgroundColor: COLORS.surface, borderColor: COLORS.line, marginBottom: 14 }}>
        ...
        <View className="flex-row flex-wrap gap-2">
          {ROLES.map(r => {
            ...
            return (
              <View key={r.key} className="rounded-xl p-2.5" style={{ width: '47%', backgroundColor: (COLORS[r.key as keyof typeof COLORS] as string) + '14' }}>
                <RoleGlyph role={r.key} color={COLORS[r.key as keyof typeof COLORS] as string} size={14} />
...
function TaskCard({ task, persona, onPress }: { task: Task; persona: Persona; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.7}
      ...
      <View className="flex-row items-center">
        <Avatar member={holder} size="m" />
        {doer && doer.id !== holder?.id && <View style={{ marginLeft: -6 }}><Avatar member={doer} size="m" ring /></View>}
      </View>
    </TouchableOpacity>
  );
}
